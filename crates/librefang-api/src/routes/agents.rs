...
/// is enabled, route every inline `ContentBlock::Image` through
/// `MediaEngine::describe_image` (default `gemini-2.5-flash`) and prepend a
/// `<image_description>` text block before the inline image. Counterpart of
/// the channel-bridge helper for the upload + stream injection path
/// (`POST /api/agents/{id}/upload` followed by `/message{,/stream}` with
/// file_id references): images arrive here pre-decoded as base64 in the
/// attachment registry, never going through the bridge's
/// `download_image_to_blocks`, so the bridge-side enrichment never fires and
/// the primary LLM ends up doing its own OCR on the inline image —
/// fabricating weekdays / dates / prices on small in-image text.
///
/// On `Ok(None)` (config disabled) or `Err` (provider failure / 30s timeout)
/// the image passes through unannotated; raw provider reason is logged but
/// never reaches the LLM prompt.
    ...
    for block in blocks {
        ...
        } = block
        {
            let attachment = librefang_types::media::MediaAttachment {
                ...
                // chars). Only used by the kernel's size cap pre-check.
