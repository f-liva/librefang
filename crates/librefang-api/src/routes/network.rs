...
pub async fn mcp_http(
    ...
    //
    // Custom branch note: upstream b17c4198 routes `handle_mcp_request`
    // through `librefang_kernel::mcp_server`; custom hasn't received
    // that kernel-side relocation yet, so the call here stays on
    // `librefang_runtime::mcp_server` (the module location on this
    // branch). The filter wiring is what matters for #5101 and is
    // identical.
    ...
    let response = librefang_runtime::mcp_server::handle_mcp_request(&request, &tools_view).await;
    Json(response)
}
