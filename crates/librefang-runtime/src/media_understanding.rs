...
impl MediaEngine {
    ...
    ///
    /// Reads the image bytes from the attachment's source (file path or
    /// base64), dispatches to the provider-specific transport selected by
    /// `[media].image_provider`, and returns the model's text description.
    ///
    /// Pre-fix this function was a stub that returned the literal placeholder
    /// `"[Image description would be generated by {provider} provider]"` —
    /// downstream callers (PR #5239's `enrich_image_blocks_with_description`,
    /// the agent-facing `media_describe` tool) saw a non-empty string and
    /// happily prepended it as a `<image_description>` block, so the primary
    /// LLM read the placeholder as a successful OCR and fabricated arbitrary
    /// in-image text (live repro 2026-05-18: agent invented "ERDO SEM
    /// Wireless Earbuds, £30.00, oggi spedito" from a real Amazon receipt
    /// screenshot it never actually OCR'd, then created a calendar event with
    /// the fabricated contents).
    ///
    /// Provider dispatch is delegated to a `match` over the configured
    /// `image_provider` string; transports without an implementation return
    /// `Err` rather than a placeholder so callers fall back to the
    /// "image passes through unannotated" branch instead of feeding garbage
    /// into the prompt.
    pub async fn describe_image(
        ...
        let provider = self
            ...
            .ok_or(
                "No vision-capable provider configured. Set [media].image_provider in config.toml.",
            )?;

        let _permit = self.semaphore.acquire().await.map_err(|e| e.to_string())?;

        let image_bytes: Vec<u8> = match &attachment.source {
        ...
        let description = describe_image_via_provider(provider, model, image_bytes, &mime).await?;
...
/// Provider-agnostic dispatch for image description. Adding a new vision
/// provider means adding one match arm here plus the transport function;
/// callers stay unaware of which provider answered. Returns `Err` for any
/// configured provider that does not yet have a transport so the channel
/// bridge falls back to "image passes through unannotated" instead of
/// feeding a placeholder string into the prompt (see the 2026-05-18 GLM-4.7
/// fabrication incident).
async fn describe_image_via_provider(
    provider: &str,
    model: &str,
    image_bytes: Vec<u8>,
    mime: &str,
    ...
    match provider {
        // Google multimodal `:generateContent` shape — works for both the
        // free `gemini-2.5-flash` and the paid `gemini-2.5-pro` model
        // identifiers without further code changes.
        "gemini" => transport_google_multimodal_describe_image(model, image_bytes, mime).await,
        // Anthropic Messages API multimodal — placeholder until the
        // transport is added; returning `Err` is intentional so callers
        // skip the description block instead of getting a stub string.
        "anthropic" => Err(format!(
            "Image description transport for provider '{}' is not yet implemented",
            provider
        )),
        // OpenAI Responses API multimodal — same intentional `Err`
        // behaviour as the Anthropic branch above.
        "openai" => Err(format!(
            "Image description transport for provider '{}' is not yet implemented",
            provider
        )),
        other => Err(format!(
            "Unsupported image description provider: '{}'. Configure [media].image_provider to a wired provider.",
            other
        )),
    }
}

/// Multimodal vision call against the Google `:generateContent` REST shape.
/// Mirrors the audio transcription transport at `gemini_transcribe` below —
/// same auth (`GEMINI_API_KEY` / `GOOGLE_API_KEY`), same base URL, same
/// JSON envelope with `inline_data` + a directive `text` part. Kept as a
/// transport helper rather than a method so other providers can be added
/// alongside it without touching the public `describe_image` signature.
async fn transport_google_multimodal_describe_image(
    model: &str,
    image_bytes: Vec<u8>,
    mime: &str,
) -> Result<String, String> {
    ...
    let body = serde_json::json!({
        "contents": [{
            "parts": [
                ...
                {
                    "text": "Describe this image in detail. Read every piece of text you see verbatim — do not paraphrase or normalize numbers, dates, or names. If you cannot read part of the text, say so explicitly. Do not invent content."
                }
            ]
        }]
    });
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| format!("Image description request failed: {}", e))?;

    if !resp.status().is_success() {
        let status = resp.status();
        let err_body = resp.text().await.unwrap_or_default();
        return Err(format!(
            "Image description API error ({}): {}",
            status, err_body
        ));
    }
    ...
    let json: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| format!("Failed to parse image description response: {}", e))?;

    json["candidates"][0]["content"]["parts"][0]["text"]
        .as_str()
        .map(|s| s.to_string())
        .ok_or_else(|| "Image description provider returned no text".to_string())
}
